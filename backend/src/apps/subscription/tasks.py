...
def refresh_subscription_statuses_task(self):
    ...
    import asyncio
    from src.db.session import get_session as get_async_session
        ...
        async with get_async_session() as db:
            ...
            for days_out in reminder_offsets:
                ...
                for sub in subs:
                    ...
                    try:
                        # ``notification`` package is optional in some contexts
                        from src.apps.notification.services.email import send_email  # type: ignore[import]
                        await send_email(
