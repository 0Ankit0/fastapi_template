...
from src.apps.iam.rbac.enforcer import CasbinEnforcer
from src.apps.iam.rbac.service import check_permission, resolve_authorization_domain
...
async def get_current_rbac_admin(
    current_user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
) -> User:
    if current_user.is_superuser:
