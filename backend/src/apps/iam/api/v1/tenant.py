...
from src.apps.iam.models.tenant import (
    InvitationStatus,
    Tenant,
    TenantInvitation,
    TenantMember,
...
async def get_tenant(
    ...
    tid = decode_id_or_404(tenant_id)
    await _require_tenant_role(tid, current_user, db, min_role=TenantRole.MEMBER)
    tenant = await _get_tenant_or_404(tid, db)
...
async def update_tenant(
    ...
    tid = decode_id_or_404(tenant_id)
    await _require_tenant_role(tid, current_user, db, min_role=TenantRole.ADMIN)
    tenant = await _get_tenant_or_404(tid, db)
...
async def delete_tenant(
    ...
    tid = decode_id_or_404(tenant_id)
    await _require_tenant_role(tid, current_user, db, min_role=TenantRole.OWNER)
    tenant = await _get_tenant_or_404(tid, db)
...
async def list_members(
    ...
    total = (
        await db.execute(
            select(func.count(col(TenantMember.id))).where(TenantMember.tenant_id == tid)
        )
    ).scalar_one()
    ...
    items = (
        await db.execute(
            select(TenantMember)
            .where(TenantMember.tenant_id == tid)
            .offset(skip)
            .limit(limit)
        )
    ).scalars().all()
...
async def update_member_role(
    ...
    await CasbinEnforcer.remove_role_for_user(str(uid), membership.role, tenant.slug)
    membership.role = data.role
    await CasbinEnforcer.add_role_for_user(str(uid), data.role, tenant.slug)
...
async def invite_member(
    ...
    tid = decode_id_or_404(tenant_id)
    await _require_tenant_role(tid, current_user, db, min_role=TenantRole.ADMIN)
    await _get_tenant_or_404(tid, db)

    # Check for active pending invitation for this email
    existing = (
        await db.execute(
            select(TenantInvitation).where(
                TenantInvitation.tenant_id == tid,
...
async def list_invitations(
    ...
    items = (
        await db.execute(
            ...
            .where(TenantInvitation.tenant_id == tid)
            .offset(skip)
            .limit(limit)
        )
    ).scalars().all()
