...
async function registerWebPush(pushConfig: PushConfigResponse): Promise<NotificationDeviceCreate | null> {
...
async function registerFcm(pushConfig: PushConfigResponse): Promise<NotificationDeviceCreate | null> {
...
async function registerOneSignal(pushConfig: PushConfigResponse): Promise<NotificationDeviceCreate | null> {
...
export function TemplateRuntimeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const syncDevice = async () => {
      let payload: NotificationDeviceCreate | null = null;

      if (activePushProvider === 'webpush') {
        payload = await registerWebPush(pushConfig);
      } else if (activePushProvider === 'fcm') {
        payload = await registerFcm(pushConfig);
      } else if (activePushProvider === 'onesignal') {
        payload = await registerOneSignal(pushConfig);
      }
